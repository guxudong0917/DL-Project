...
可以这样理解：H+2P是填充后的图像，减去最开始的卷积核，剩下的除以S就是除去第一次要滑动多少次。如果是stride为1时，想要大小不变，就让2*P+1=kernel大小
...
早期的CNN是每层都池化的，但现代的CNN结构选择隔几层池化一次，因为这样可以减少信息的损失，每层都池化特征会消失的很快。一般在conv层大小不变，pool层下采样

随着网络加深，特征从低级局部模式向高级语义模式演化，空间分辨率不断降低。为了保持信息表达能力并刻画更丰富的特征组合，CNN 通常在更深层引入更多的滤波器（通道）